...
object Par {
  ...
  /* Ex 7.3 */
  def map2WithTimeouts[A,B,C](a: Par[A], b: Par[B])(f: (A,B) => C)(timeout: Long, units: TimeUnit): Par[C] = {
    (es: ExecutorService) =>
      val af = a(es)
      val bf = b(es)
      val nanos = units.convert(timeout, TimeUnit.NANOSECONDS)
      val max = System.nanoTime() + nanos
      val ra = af.get(timeout, units)
      val rb = bf.get(max - System.nanoTime(), TimeUnit.NANOSECONDS)
      UnitFuture(f(ra, rb))
  }
  ...
  def choiceViaChooser[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] =
    chooser(cond)(res => if (res) t else f)

  /* Ex 7.13 iii */
  def choiceNViaChooser[A](n: Par[Int])(choices: List[Par[A]]): Par[A] =
    chooser(n)(res => choices(res))
